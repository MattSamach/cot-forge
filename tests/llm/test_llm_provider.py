...
class TestRetryLogic(unittest.TestCase):
  ...
  def test_no_retry_on_other_exceptions(self, mock_types, mock_client):
    ...
    self.assertEqual(
        mock_client.return_value.models.generate_content.call_count, 1)
...
class TestTokenLimits(unittest.TestCase):
  ...
  def test_token_limit_checks(self):
    ...
    self.assertTrue(self.provider.check_token_limits(
        prompt="Test prompt", max_tokens=10))
  ...
  def test_input_token_limit_exceeded(self):
    ...
    self.assertIn("Estimated input token limit exceeded",
  ...
  def test_output_token_limit_exceeded(self):
    ...
    self.assertIn("Estimated output token limit exceeded",
                  str(context.exception))
