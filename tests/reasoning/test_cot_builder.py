...
class TestCoTBuilder(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.mock_post_processor.process_result = MagicMock(
        return_value={"id": "test_id", "content": "test reasoning"})
    ...
    self.mock_post_processing_llm.to_dict = MagicMock(
        return_value={"type": "MockPostProcessingLLM"})
    self.mock_search.to_dict = MagicMock(return_value={"name": "MockSearch"})
    self.mock_verifier.to_dict = MagicMock(
        return_value={"type": "MockVerifier"})
    ...
    with patch('cot_forge.persistence.PersistenceManager.save_config'), \
      ...
      self.mock_persistence.generate_question_id = MagicMock(
  ...
  def test_batch_processing_persistence(self):
    ...
    self.cot_builder.process = MagicMock(return_value=(
        self.mock_search_result, {"id": "test_id", "content": "test"}))
  ...
  def test_integration_with_real_persistence(self):
    ...
    with patch('cot_forge.persistence.persistence_manager.PersistenceManager.save_result') as mock_save_result:
      ...
      mock_post_processor.process_result = MagicMock(
          return_value={"id": "test_id", "content": "test reasoning"})
  ...
  def test_skip_processed_questions(self):
    ...
    self.mock_persistence.should_skip.assert_called_once_with(
  ...
  def test_process(self, mock_logger):
    ...
    # Mock dependencies
    self.cot_builder.build_cot = MagicMock(
        return_value=self.mock_search_result)
    self.mock_persistence.generate_question_id = MagicMock(
        return_value="test-id")
    self.mock_post_processor.process_result = MagicMock(
        return_value={"id": "test-id", "content": "test reasoning"})
